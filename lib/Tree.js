...
class Tree {
  ...
  async getPackages() {
    ...
    const rootLockSpec = getRootLockSpec(lock);

    const allPackages = new Set();
    for (const packFilepath of packageFilepaths) {
      // eslint-disable-next-line no-await-in-loop
      const pack = await this.getJsonFile(packFilepath);
      const relpath = path.posix.relative(this.cwd, packFilepath);
      const reldirpath = path.posix.dirname(relpath);
      for (const depType of [
        ...
        if (deps) {
          for (const depName of Object.keys(deps)) {
            const depLockSpec = rootLockSpec.get(depName, reldirpath);
            const depPackage = this.getPackage(depLockSpec);
