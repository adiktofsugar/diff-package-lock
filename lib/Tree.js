  ...
  async getFile(filepath) {
    ...
    try {
      const stdout = execSync(`git show "${this.treeish}:${filepath}"`, {
        cwd: this.cwd,
        stdio: "pipe" // not sure why i need this, but without it the error will propagate to the console
      }).toString();
      return JSON.parse(stdout);
    } catch (e) {
      // 'fatal: Path \'lerna.json\' does not exist in \'react-15\'',
      const stderr = e.stderr.toString();
      const isMissing = /^fatal: path .+ does not exist in .+/i.test(stderr);
      if (e.status === 128 && isMissing) {
        throw error;
      }
      throw e;
    }
